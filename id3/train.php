...
function id3($data, $origin, $features, $target_attribute_name = 'hasil', $parent_node_class = NULL)
{
  ...
  if (count($uniq) <= 1) {
    ...
    foreach (array_unique($data_best) as $v) {
      ...
      $subtree = id3($sub_data, $origin, $new_features, $target_attribute_name, $parent_node_class);
...
define("T", "T");
define("B", "B");
$tree = id3($dataset, $dataset, $g_features);
pprint($tree);
